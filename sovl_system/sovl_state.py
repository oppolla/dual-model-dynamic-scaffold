...
class SOVLState:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.scaffold_unk_id = config.get("scaffold_unk_id", 0)  # Should be set from tokenizer
    ...
    def from_dict(self, data: Dict[str, Any]):
        ...
        self.dream_memory = deque(
            [(torch.tensor(m, dtype=torch.float32), w) for m, w in data.get("dream_memory", [])],
    ...
    def prune_dream_memory(self, threshold: float):
    ...
    def decay_memory_weights(self, decay_rate: float):
