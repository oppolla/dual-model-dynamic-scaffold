...
class SOVLTrainer:
    def __init__(
        ...
        # Initialize CuriosityManager with config parameters
        self.curiosity_manager = CuriosityManager(
            weight_ignorance=config.curiosity_weight_ignorance,
            weight_novelty=config.curiosity_weight_novelty,
            pressure_threshold=config.curiosity_pressure_threshold,
            pressure_drop=config.curiosity_pressure_drop,
            novelty_threshold_spontaneous=config.curiosity_novelty_threshold_spontaneous,
            novelty_threshold_response=config.curiosity_novelty_threshold_response,
            silence_threshold=config.curiosity_silence_threshold,
            question_cooldown=config.curiosity_question_cooldown,
            queue_maxlen=config.curiosity_queue_maxlen,
            max_new_tokens=config.curiosity_max_new_tokens,
            base_temperature=config.curiosity_base_temperature,
            temperament_influence=config.curiosity_temperament_influence,
            top_k=config.curiosity_top_k
        )
    ...
    def _dream(self):
        """Generate dream prompts, update dream memory, and generate curiosity-driven questions."""
        ...
        # Choose dream entry with curiosity weighting
        if self.config.enable_prompt_driven_dreams:
            ...
            for i, entry in enumerate(log_entries):
                ...
                curiosity_score = self.curiosity_manager.compute_curiosity(
                    base_conf=self.state.confidence_history[-1] if self.state.confidence_history else 0.5,
                    scaf_conf=self.state.sleep_confidence_sum / self.state.sleep_confidence_count if self.state.sleep_confidence_count > 0 else 0.5,
                    memory_embeddings=list(self.state.dream_memory),
                    query_embedding=log_hidden.detach().cpu()
                )
                weight = (
                    self.config.dream_prompt_weight * similarity +
                    (1 - self.config.dream_prompt_weight) * recency +
                    curiosity_score * 0.5  # Amplify curiosity influence
                )
        ...
        # Compute curiosity for memory weight
        dream_inputs = self.tokenizer(
            dream_prompt,
            return_tensors="pt",
            padding=True,
            truncation=True,
        ...
        with torch.no_grad():
            dream_hidden = self.model(**dream_inputs, output_hidden_states=True).hidden_states[-1].mean(dim=1)
        curiosity_score = self.curiosity_manager.compute_curiosity(
            base_conf=self.state.confidence_history[-1] if self.state.confidence_history else 0.5,
            scaf_conf=self.state.sleep_confidence_sum / self.state.sleep_confidence_count if self.state.sleep_confidence_count > 0 else 0.5,
            memory_embeddings=list(self.state.dream_memory),
            query_embedding=dream_hidden.detach().cpu()
        )

        # Apply noise based on temperament and curiosity
        ...
        noise_scale = (
            ...
            curiosity_score * 0.05  # Curiosity amplifies noise
        )
        ...
        with self.memory_lock:
            ...
            weight = self.config.dream_memory_weight + curiosity_score * 0.5
            if is_novel:
...
class SOVLTrainer:
    ...
    def gestate(self, log_entries: List[dict], resume: bool = False) -> bool:
        ...
        # Generate curiosity-driven question based on last prompt
        if self.gestation_batch:
            last_prompt = self.gestation_batch[-1]["prompt"]
            question = self.curiosity_manager.generate_question(
                state=self.state,
                tokenizer=self.tokenizer,
                model=self.model,
